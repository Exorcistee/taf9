...
vector<string> ReadAllLines(const string& inputFile)
{
	...
	if (!file.is_open())
	{
		throw runtime_error("error ёлки палки: " + inputFile);
	}
	string line;
...
vector<string> CombinedLines(const vector<string>& lines)
{
	vector<string> combineLines;
	string currentLine = "";
	...
	for (const string& line : lines)
	{
		...
		if (regex_search(line, rulePattern))
		{
			...
			{
				combineLines.push_back(currentLine);
				currentLine.clear();
			}
			currentLine = line;
		}
